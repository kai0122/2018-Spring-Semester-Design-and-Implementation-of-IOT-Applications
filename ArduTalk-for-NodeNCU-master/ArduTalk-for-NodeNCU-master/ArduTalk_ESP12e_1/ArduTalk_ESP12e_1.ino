...
#define DM_name  "KAI_MERGE2_IN" 
#define DF_list  {"KAI_MERGE2_IN0", "KAI_MERGE2_IN1", "KAI_MERG2_IN2", "KAI_MERGE2_IN3"}
...
String scan_network(void){
    ...
    AP_List += "<option value=\"\" disabled selected>��蹓蕭嚙踝蕭</option>";
...
void setup() {
    pinMode(2, INPUT_PULLUP);// D4 : on board led
...
void loop() {
    ...
    if (digitalRead(5) == LOW) { //  D0
      Serial.println ("D0\n");
      push("KAI_MERGE2_IN0", String("1"));
    }
    else{
      //push("KAI_MERGE_IN1", String("0"));
    }
    if (digitalRead(4) == LOW) {  // D1
      Serial.println ("PRESSED D0\n");
      push("KAI_MERGE2_IN0", String("2"));
    }
    else{
      //push("KAI_IN1", String("0"));
    }
    if (digitalRead(0) == LOW) {  //  D2
      Serial.println ("PRESSED D8\n");
      push("KAI_MERGE2_IN0", String("3"));
    }
    else{
      //push("KAI_IN2", String("0"));
    }
    if (digitalRead(2) == LOW) {  //  D3
      Serial.println ("PRESSED\n");
      push("KAI_MERGE2_IN0", String("4"));
    }
    else{
      //push("KAI_IN3", String("0"));
    }

    
    /*
    if (digitalRead(16) == LOW) { //  D0
      //Serial.println ("D0\n");
      push("KAI_IN0", String("1"));
    }
    else{
      //push("KAI_IN0", String("0"));
    }
    if (digitalRead(5) == LOW) {  // D1
      //Serial.println ("PRESSED D0\n");
      push("KAI_IN1", String("1"));
    }
    else{
      //push("KAI_IN1", String("0"));
    }
    if (digitalRead(4) == LOW) {  //  D2
      //Serial.println ("PRESSED D8\n");
      push("KAI_IN2", String("1"));
    }
    else{
      //push("KAI_IN2", String("0"));
    }
    if (digitalRead(0) == LOW) {  //  D3
      //Serial.println ("PRESSED\n");
      push("KAI_IN3", String("1"));
    }
    else{
      //push("KAI_IN3", String("0"));
    }
    if (digitalRead(2) == LOW) {  //  D4
      //Serial.println ("PRESSED\n");
      push("KAI_IN4", String("1"));
    }
    else{
      //push("KAI_IN4", String("0"));
    }
    if (digitalRead(14) == LOW) {  // D5
      //Serial.println ("PRESSED\n");
      push("KAI_IN5", String("1"));
    }
    else{
      //push("KAI_IN5", String("0"));
    }
    if (digitalRead(12) == LOW) { //  D6
      //Serial.println ("PRESSED\n");
      push("KAI_IN6", String("1"));
    }
    else{
      //push("KAI_IN6", String("0"));
    }
    if (digitalRead(13) == LOW) { //  D7
      //Serial.println ("PRESSED\n");
      push("KAI_IN7", String("1"));
    }
    else{
      //push("KAI_IN7", String("0"));
    }
    if (digitalRead(15) == LOW) { //  D8
      //Serial.println ("D8\n");
      push("KAI_IN8", String("1"));
    }
    else{
      //push("KAI_IN8", String("0"));
    }
    */
     delay(200);
    //Serial.println ("IN\n");
    ...
    if (millis() - cycleTimestamp > 200) {
        ...
        if (result != "___NULL_DATA___"){
            ...
            if (result.toInt() > 0 ) digitalRead(14);
            else digitalRead(14);
        }
        ...
        if (result != "___NULL_DATA___"){
            ...
            if (result.toInt() > 0 ) digitalRead(12);
            else digitalRead(12);
        }
        ...
        if (result != "___NULL_DATA___"){
            ...
            if (result.toInt() > 0 ) digitalRead(13);
            else digitalRead(13);
        }
        ...
        if (result != "___NULL_DATA___"){
            ...
            if (result.toInt() > 0 ) digitalRead(15);
            else digitalRead(15);
        }
    ...
    if (millis()-LEDonCycle > 1) digitalRead(2);
  */
}
